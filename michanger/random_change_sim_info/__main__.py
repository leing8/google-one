"""
Random & Change SIM Info Only CLI 入口。

支持 python -m michanger.random_change_sim_info 运行。
在 TWRP Recovery 中替换 mi_info.json 实现 SIM 信息随机化。

用法：
    python -m michanger.random_change_sim_info
    python -m michanger.random_change_sim_info -s SERIAL_NUMBER -v
    python -m michanger.random_change_sim_info --profile blazer
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="michanger.random_change_sim_info",
        description=(
            "Random & Change SIM Info Only — "
            "SIM 信息随机化（仅替换 mi_info.json）"
            "（严格还原 14.0-Random & Change SIM Info Only.pcapng）"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "示例：\n"
            "  python -m michanger.random_change_sim_info\n"
            "  python -m michanger.random_change_sim_info -s SERIAL_NUMBER\n"
            "  python -m michanger.random_change_sim_info --profile blazer\n"
            "  python -m michanger.random_change_sim_info -v\n"
        ),
    )
    parser.add_argument(
        "--adb-path",
...
def _print_result(result: ChangeSIMResult) -> None:
    """打印执行结果。
    ...
    print()
    print("=" * 60)
    print(f"  Random & Change SIM Info Only 结果: {status}")
    print("=" * 60)
    ...
    if result.third_party_packages:
        print()
        print(f"  第三方应用 ({len(result.third_party_packages)}):")
...
def main() -> None:
    ...
    try:
        result = random_change_sim_info(
            adb_path=args.adb_path,
            serial=serial,
            profile_name=args.profile,
        )
    except AdbError as exc:
        log.error("Random & Change SIM Info Only 失败: %s", exc)
        sys.exit(1)
